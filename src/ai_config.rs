...
pub fn parse(text: &str) -> Result<AiConfig, String> {
    ...
    for p in &cfg.platforms {
        if p.caps.is_empty() {
            return Err(format!("platform '{}': needs at least one (tokens, duration) cap", p.name));
        }
        for (i, (t, d)) in p.caps.iter().enumerate() {
            if *t == 0 { return Err(format!("platform '{}': cap {i} token count must be > 0", p.name)); }
            if *d == 0 { return Err(format!("platform '{}': cap {i} duration must be > 0", p.name)); }
            if i > 0 {
                let (pt, pd) = p.caps[i - 1];
                if *t <= pt {
                    return Err(format!("platform '{}': caps not monotonic (tokens[{i}]={t} <= tokens[{}]={pt})", p.name, i - 1));
                }
                if *d <= pd {
                    return Err(format!("platform '{}': durations not monotonic (duration[{i}]={d} <= duration[{}]={pd})", p.name, i - 1));
                }
            }
        }
    }
