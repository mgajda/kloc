...
fn detect_java_extension() {
    if let Some(lang) = detect("Main.java", None) {
        assert_eq!(lang, "Java");
    }
}
...
fn detect_haskell_extension() {
    if let Some(lang) = detect("Main.hs", None) {
        assert_eq!(lang, "Haskell");
    }
}

#[test]
fn detect_ocaml_extension() {
    if let Some(lang) = detect("main.ml", None) {
        assert_eq!(lang, "OCaml");
    }
}

#[test]
fn detect_go_extension() {
    if let Some(lang) = detect("main.go", None) {
        assert_eq!(lang, "Go");
    }
}
...
fn detect_rust_vs_other() {
    assert_eq!(detect("main.rs", None), Some("Rust"));
    if let Some(lang) = detect("main.rs", None) {
        assert_ne!(lang, "C++");
    }
}
...
fn detect_makefile_exact_filename() {
    if let Some(lang) = detect("Makefile", None) {
        assert_eq!(lang, "Make");
    }
    if let Some(lang) = detect("makefile", None) {
        assert_eq!(lang, "Make");
...
fn detect_shebang_python() {
    assert_eq!(
        detect("script", Some(b"#!/usr/bin/env python3")),
        Some("Python")
    );
}
...
fn detect_shebang_perl() {
    if let Some(lang) = detect("script", Some(b"#!/usr/bin/perl -w")) {
        assert_eq!(lang, "Perl");
    }
}
...
fn detect_shebang_ruby() {
    if let Some(lang) = detect("script", Some(b"#!/usr/bin/env ruby")) {
        assert_eq!(lang, "Ruby");
    }
}
...
fn detect_shebang_node() {
    assert_eq!(
        detect("script", Some(b"#!/usr/bin/env node")),
        Some("JavaScript")
    );
}
...
fn detect_elixir_extension() {
    if let Some(lang) = detect("app.ex", None) {
        assert_eq!(lang, "Elixir");
    }
}
...
fn detect_shebang_fish() {
    ...
    if let Some(lang) = detect("script", Some(b"#!/usr/bin/env fish")) {
        assert_eq!(lang, "Fish");
    }
}
...
fn detect_verilog_extension() {
    if let Some(lang) = detect("module.sv", None) {
        assert_eq!(lang, "Verilog");
    }
    if let Some(lang) = detect("module.v", None) {
        assert_eq!(lang, "Verilog");
    }
}
...
fn detect_v_or_verilog() {
    ...
    assert!(
        r.is_none() || r == Some("Verilog") || r == Some("V"),
        "unexpected language: {r:?}"
    );
}

#[test]
fn detect_dart_extension() {
    if let Some(lang) = detect("main.dart", None) {
        assert_eq!(lang, "Dart");
    }
}
...
fn detect_lua_extension() {
    if let Some(lang) = detect("script.lua", None) {
        assert_eq!(lang, "Lua");
    }
}
...
fn detect_fortran_extension() {
    if let Some(lang) = detect("program.f90", None) {
...
fn detect_perl_extension() {
    if let Some(lang) = detect("script.pl", None) {
        assert_eq!(lang, "Perl");
    }
}
...
fn detect_php_extension() {
    if let Some(lang) = detect("index.php", None) {
        assert_eq!(lang, "PHP");
    }
}
...
fn detect_swift_extension() {
    if let Some(lang) = detect("main.swift", None) {
...
fn detect_ruby_extension() {
    if let Some(lang) = detect("script.rb", None) {
        assert_eq!(lang, "Ruby");
    }
}
...
fn detect_fish_extension() {
    if let Some(lang) = detect("script.fish", None) {
        assert_eq!(lang, "Fish");
    }
}
...
fn detect_haskell_literature() {
    if let Some(lang) = detect("file.lhs", None) {
        assert_eq!(lang, "Haskell");
    }
}
...
fn test_all_languages_have_unique_extensions() {
    let registry = kloc::language::registry();
    let mut seen_exts: std::collections::HashMap<&str, Vec<&str>> =
